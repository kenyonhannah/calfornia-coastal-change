...
def main():
    ...
    filepath = input('CoastSat Raw Transect Data Filepath: ')
...
def plot_transect_data(df_prepped, df_reclassed, df_trend, df_detrended, beachID):
    ...
    transect_base = "usa_CA_" + beachID + "-"
    transect = transect_base + transect_number
...
def prepare_data(df):
    ...
    # drop any data from before 2000
    df = df[df['timestamp'].dt.year >= 2000]

    # reset the index so index starts at 0
    df = df.reset_index(drop=True)
